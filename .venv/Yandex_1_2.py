# A. Интеллектуальный вендинг
from sys import flags

money = input()
papers = int(money.split()[0])
coins = int(money.split()[1])
print(f'Банкнот {papers}, монет {coins}')
vending_machine = input()
bottle_price = int(vending_machine.split()[0])
coins_4_change = int(vending_machine.split()[1])
print(f'Цена бутылки {bottle_price}, сдача в автомате {coins_4_change}')

wallet = papers * 1000000 + coins
print (wallet)
payment_paper = bottle_price // 1000000
payment_coins = bottle_price - payment_paper * 1000000
print(f'Для платежа нужно {payment_paper}, {payment_coins}')

def purchase(payment_paper, payment_coins):
    global papers, coins
    papers -= payment_paper
    coins -= payment_coins
    return papers, coins

count = 0
flag = True

while wallet >= bottle_price and flag == True:
    print(f'Денег перед покупкой - {wallet}')
    check = input()
    if coins >= bottle_price:
        papers, coins = purchase(0, bottle_price)
        print(papers, coins)
        coins_4_change += bottle_price
        count += 1
        wallet -= bottle_price
        print(f'1. Осталось  после покупки денег {wallet}, в кошельке банкнот - {papers}'
              f', монет - {coins}, сдачи в автомате - {coins_4_change}')
        print(f'Купил {count} бутылки')
        continue
    elif payment_paper == 0 and papers > 1 and coins < bottle_price and coins_4_change >= (1000000 - bottle_price):
        papers, coins = purchase(1, 0)
        print(papers, coins)
        change = 1000000 - bottle_price
        print(change)
        coins += change
        coins_4_change -= change
        count += 1
        wallet -= bottle_price
        print(f'2. Осталось  после покупки денег {wallet}, в кошельке банкнот - {papers}'
              f', монет - {coins}, сдачи в автомате - {coins_4_change}')
        print(f'Купил {count} бутылки')
        continue
    elif coins < payment_coins and papers > 1 and coins_4_change >= (1000000 - bottle_price):
        papers, coins = purchase(payment_paper + 1, 0)
        print(papers, coins)
        change = (payment_paper + 1) * 1000000 - bottle_price
        print(change)
        coins += change
        coins_4_change -= change
        count += 1
        wallet -= bottle_price
        print(f'3. Осталось  после покупки денег {wallet}, в кошельке банкнот - {papers}'
              f', монет - {coins}, сдачи в автомате - {coins_4_change}')
        print(f'Купил {count} бутылки')
        continue
    elif coins < bottle_price and payment_paper >= 1 and papers >= 1:
        papers, coins = purchase(payment_paper, payment_coins)
        print(papers, coins)
        count += 1
        coins_4_change += payment_coins
        wallet -= bottle_price
        print(f'4. Осталось  после покупки денег {wallet}, в кошельке банкнот - {papers}'
              f', монет - {coins}, сдачи в автомате - {coins_4_change}')
        print(f'Купил {count} бутылки')
        continue
    else:
        flag = False

print(f'Купил {count} бутылки')
print (count)